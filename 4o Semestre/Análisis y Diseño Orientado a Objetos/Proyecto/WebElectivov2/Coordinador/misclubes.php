				...
				<div class="w3-half w3-container">
					<div class="w3-container">
						...
						<?php
							...
							$conectar = conectar();
							if ($conectar) {
								$consulta ="select c.idClub as idClub,c.nombre as nombre, r.cupo as cupo from club c, registrado r, coordinador_has_club cc where c.idClub = r.idClub
								    and c.idEje = r.idEje and r.idclub=cc.idclub and cc.idusuario=$idusuario;";
									...
									<tbody>
										...
										<?php
											if (isset($_GET['idClub'])) {
												...
												$conectar = conectar();
												if ($conectar) {
													$consulta ="select dia, horario from horarioClub where idClub = $idClub order by dia;";
													$Registro = mysqli_query($conectar, $consulta);
													while($Ren = mysqli_fetch_array($Registro))
													{
														echo "<tr>";
