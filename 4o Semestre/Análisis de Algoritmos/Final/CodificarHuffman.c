...
void GetFrequency (FILE ** texto, elemento * frecuencias){
	...
	*texto = fopen("texto.txt","r");
	...
	do
	{
		...
		{
			(frecuencias[c-65].nFrecuencia)++;
			(frecuencias[c-65].cLetra) = c;
		}
		else if (c>=97&&c<=122)//minisculas
		{
			(frecuencias[c-97+26].nFrecuencia)++;
			(frecuencias[c-97+26].cLetra) = c;
		}
		else if (c==32)//espacio
		{
			(frecuencias[52].nFrecuencia)++;
			(frecuencias[52].cLetra) = c;
		}
		i++;
	}	while(c!=EOF);
...
int main (void)
{
 	...
 	while(c!=EOF)
	{
		...
		if (c>=65&&c<=90)//mayusculas
		{
			WriteBinary((frecuencias[c-65]), &final,binario);
		}
		else if (c>=97&&c<=122)//minisculas
		{
			WriteBinary((frecuencias[c-97+26]),&final, binario);
		}
		else if (c==32)//espacio
		{
			WriteBinary(frecuencias[52],&final, binario);
		}
