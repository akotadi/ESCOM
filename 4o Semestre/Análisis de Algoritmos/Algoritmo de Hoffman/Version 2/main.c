...
int main (void)
{
	...
	int total = 0 ;
	for(i = 0 ; i < tam ; i ++){
		total += longitud[bytes[i]];
	}
	int original = total;
	total = (total/8)+1;

	unsigned char *comp;
	comp = (unsigned char*)malloc(total*sizeof(unsigned char));	
	int ultimo = 0; 
	int pb = 0;
	for(i = 0 ; i < tam ;  i++){
		int j = 0 ;
		for(j = 0 ; j < longitud[bytes[i]]; j++){
			if(codigo[bytes[i]][j] == 1){
	...
	fwrite(comp,total+1 , 1, salida);
	fseek(salida, 0, SEEK_SET);	
	fclose(salida);

	salida = fopen("parametros.txt","w");
	fprintf(salida,"%d\n",tam);
	fprintf(salida,"%d\n",original);	
	for(i = 0 ; i< 256 ; i ++){
		fprintf(salida,"%d\n", veces[i]);
