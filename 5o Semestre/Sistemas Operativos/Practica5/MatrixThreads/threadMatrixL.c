...
void* executeThread(void *lpParam){
...
int main()
{
	...
	for(i = 0; i < 5; ++i){
		data[i].nSize = nSize;
		data[i].nOption = i;
		pthread_create(&aThread[i], NULL, executeThread, (void*)(&data[i]));
	}
	...
	data[5].nSize = nSize;
	data[5].nOption = 5;
	pthread_create(&aThread[5], NULL, executeThread, (void*)(&data[5]));
	pthread_join(aThread[5], NULL);
