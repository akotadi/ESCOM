...
// OBJETO sesion PARA ALMACENAR DATOS ENTRE Servlets
HttpSession sesion ;
...
// GUARDAMOS EL ORIGEN Y EL DESTINO EN EL OBJETO sesion
sesion = entrada.getSession() ;
sesion.putValue("origen",origen);
sesion.putValue("destino",destino);
...
//       os.println("<INPUT TYPE=HIDDEN NAME=destino VALUE=\""+destino+ "\"> ");
  ...
  String costoFormateado = precision2.format( 
       ...
       os.println("   <TD> <CENTER> <INPUT TYPE=CHECKBOX NAME=cc"+cuenta+"> </CENTER> </TD>");
        String linea = resultado.getString("Linea");
       os.println("   <TD> " 
       + " <a href=\"http://localhost:8080/imagenes/ejemplo.pdf\">"
       + linea
       + "</a>" 
       + 
  ...
  String costoFormateado = precision2.format( 
       ...
       int disponible =  ( Integer.parseInt(cupo) - Integer.parseInt(vendido) ); 
            ...
            os.println("   <TD> <img src=\"http://localhost:8080/imagenes/azul.jpg\"> </TD>");  
            os.println("</TR>");
            cuenta++;
       }
      os.println("</TABLE>");
