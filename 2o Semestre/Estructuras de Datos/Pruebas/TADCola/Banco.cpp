...
int main(){
	...
	cola* cajeras;
	int* tiempo_cajeras;
	int tc,tu,tp,tiempo=0,clientes_cont=0,clientes_atend=0,cont_usu=0,n,cajera_num;
	elemento e;

	printf("Numero de cajeras: \n");
	scanf("%i",&n);
	
	tiempo_cajeras=(int*)malloc(n*sizeof(int));//Se crea un arreglo para almacenar el tiempo que le toma a cada cajera atender
	...
	while(clientes_atend < 7){
		...
		//printf("Trono aqui 0\n");
		for (int i = 0; i < n; ++i){
			if((TIEMPO_BASE*tiempo) % tiempo_cajeras[i]   == 0){//Se va recorriendo cada una de las cajas para ver si ya estan en tiempo de desencolar
