...
void Mostrar(int n,int* ejecucion,cola* cajeras,cola preferentes,cola clientes,cola usuarios){
	...
	for (i = 0; i < n; ++i) // Iteración que muestra el estado de cada caja
	{
		printf("Caja: %i\n",i+1);
...
int CajasVacias(cola* cajeras, int n){
	int aux=0,i;
	for (i = 0; i < n; ++i){ // Iteración que recorre las cajas
...
int main(){
	...
	cola* cajeras;
	int i,n,t_cajeras,tc,tu,tp,tiempo=0,clientes_cont=0,clientes_atend=0,cont_usu=0,cajera_num;
	int* ejecucion;
	elemento e;
	...
	scanf("%i",&t_cajeras);
	t_cajeras=t_cajeras*TIEMPO_BASE;
	...
	while(1){ // Iteración que simulará la atención de los clientes
		...
		for (i = 0; i < n; ++i){ // Iteración para revisar el estado de cada caja
		...
		while(CajasVacias(cajeras,n)>0 ){ 
			// Iteración que introducirá a los clientes en espera a una caja vacía
			...
			if (Empty(&usuarios) && Empty(&preferentes) && Empty(&clientes)) // En caso de que no haya más clientes en espera, se termina la iteración
				break;
		}
