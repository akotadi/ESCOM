...
/*
recibe un arbol y su poscion 
en caso que la posicion sea una hoja retorna TRUE
en caso contrario retorna FALSE
*/
boolean es_hoja (arbol_bin *A,posicion p)
{
...
//recibe un arbol y una posicion
//regresa el binario asignado a dicho nodo
int valorbinario (arbol_bin *A, posicion p)
{
	int bin;
	if (!NullNode(A, p))
	{
...
arbol_bin unirpadre (arbol_bin *A, arbol_bin *B)
{
	
	arbol_bin C;
	C = malloc(sizeof(nodo)); //creamos nodo
	C = malloc(sizeof(nodo));
		(C)->e.frec=((*A)->e.frec) + ((*B)->e.frec);//suamos frecuencias y se lo asignamos al padre
		(C)->e.caracter='?';
		(C)->izq = *A;
		(C)->der = *B;
		(C)->der->binario=1;
