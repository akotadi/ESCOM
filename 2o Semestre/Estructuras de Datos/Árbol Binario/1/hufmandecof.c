...
arbol_bin generar_arbol (arbol_bin *letras,elemento frecuencias[])
{
	...
	for (k = 0; k < num_letras-1; k++)
	{
			...
			min1=e.frec;//toma a el más pequeño como el ultmi de la cadena para asegurarnos que no se haya usado
			min2=min1+1;
			for (i = 0; i < j; i++)
			{
				p=Root(&letras[i]);
				...
				{
					if (e.frec<min1)//se toma el más pqueño
					{
						min1=e.frec;
					}
					else if (e.frec<=min2 && e.frec>min1)//se toma el más pequeño pero mayor que m1
					{
						min2=e.frec;
					}
				}
...
int main (void)
{
	...
	for (i = 0; i < 53; i++)
	{
		fscanf(texto,"%d\n",&(frecuencias[i].frec));//se toma la frecuencia del archivo
		if (i>=0&&i<=25)//mayusculas
		{
			(frecuencias[i].caracter)=i+65;//se genera el cracter correspondiente 
		}
		else if (i>=26&&i<=51)//minisculas
		{
			(frecuencias[i].caracter)=i+97-26;//se genera el cracter correspondiente 
		}
		else if (i==52)//espacio
		{
			(frecuencias[i].caracter)=32;//se genera el cracter correspondiente 
		}
	}
	...
	while(c!='\n') 
	{
		...
		{	
			for (i = 0; i < 8; i++)
			{
				...
				if(es_hoja (&final,p))//cuando encuentre una hoja imprime el caracter y regresa a la raiz
				{
					e=ReadNode(&final, p);
					printf("%c",e.caracter);
		...
		else//si es el ultmimo cracter 
		{
			for (i = 0; i < 8-ignorados; i++)
			{
				...
				if(es_hoja (&final,p))//si encuentra una hoja imprime el caracter y regresa la posicion a la raiz del arbol
				{
					e=ReadNode(&final, p);
					printf("%c",e.caracter);
					fputc(e.caracter,descifrado);
