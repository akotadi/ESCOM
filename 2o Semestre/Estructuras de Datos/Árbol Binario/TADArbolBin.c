...
void NewTreeRightSon(arbol_bin *A, posicion p, arbol_bin *B)
{
	if(Empty(A))
	{
		*A = *B;
	...
	{
		if(p->der != NULL){
			printf("Error: NewTreeRightSon: ya existe");
			exit(1);
		}
		else
		{
		...
		printf("Error: NewTreeRightSon");
		exit(1);
	}
	return;
}

void NewTreeLeftSon(arbol_bin *A, posicion p, arbol_bin *B)
{
	if(Empty(A))
	{
		*A = *B;
	...
	{
		if(p->izq != NULL){
			printf("Error: NewTreeLeftSon: ya existe");
			exit(1);
		}
		else
		{
